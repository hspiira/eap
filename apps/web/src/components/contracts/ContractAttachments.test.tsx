...
const mocks = vi.hoisted(() => ({ list: vi.fn(), upload: vi.fn(), download: vi.fn(), canWrite: true }))
vi.mock("@/api/endpoints/documents", () => ({ documentsApi: { list: mocks.list, uploadContractAttachment: mocks.upload, download: mocks.download } }))
...
describe("contract attachments", () => {
  it("uploads to the selected contract and reloads persisted attachments", async () => {
    ...
    const document = {id: "document-1", name: "Signed agreement.pdf", file_path: "private/file", created_at: "2026-09-05", file_size: 1024}
  ...
  it("shows upload failures without a successful attachment", async () => {
    ...
    renderWithProviders(<ContractAttachments contractId="contract-1" />)
    await user.upload(screen.getByLabelText("Attach a file"), new File(["%PDF-1.7"], "agreement.pdf", {type: "application/pdf"}))
