 ...
 *
 * The batch is a whole, so the headline is one part-to-whole bar whose four
 * segments account for every row: accepted, already held, blocked, and the
 * apply-time write refusals. The blocked reasons below carry a share of the
...
const QUEUE_PRESENTATION: Record<string, { label: string; action: string; to: string }> = {
  ...
  UnmappedPractitioner: {
    label: "No practitioner of that name",
    action: "Add practitioner",
    to: "/providers",
  },
  ...
  Conflicting: {
    label: "Disagrees with what is already recorded",
    action: "Review rows",
    to: "/service-sessions",
  },
  Rejected: { label: "Undated or dated ahead", action: "Review rows", to: "/service-sessions" },
}
...
const SEGMENTS = [
  ...
  { key: "failed", label: "Write refused", color: "var(--color-destructive)" },
] as const
