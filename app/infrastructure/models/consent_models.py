...
class ConsentModel(CuidMixin, TenantMixin, Base, TimestampMixin):
    ...
    subject_clinical_subject_id: Mapped[str] = mapped_column(
    ...
    purpose_other_detail: Mapped[str | None] = mapped_column(
        Text, nullable=True
    )
    ...
    granted_by_subject_reference: Mapped[str | None] = mapped_column(
        String(255), nullable=True
    )
    signed_artifact_document_id: Mapped[str | None] = mapped_column(
...
class DataSharingRegisterEntryModel(
    CuidMixin, TenantMixin, Base, TimestampMixin
):
    ...
    scope: Mapped[ConsentScope] = mapped_column(
        EnumValueType(ConsentScope), nullable=False
    )
