...
class CrisisContactMapper:
    ...
    def to_entity(model: CrisisContactModel) -> CrisisContact:
        entity = CrisisContact(
            ...
            safety_plan_id=SafetyPlanId(model.safety_plan_id) if model.safety_plan_id else None,
            risk_level=TriageRiskLevel(model.risk_level) if model.risk_level else None,
            warm_handoff=CrisisWarmHandoff(model.warm_handoff),
            dispatched_at=ensure_utc(model.dispatched_at) if model.dispatched_at else None,
            outcome=CrisisContactOutcome(model.outcome) if model.outcome else None,
    ...
    def to_model(entity: CrisisContact) -> CrisisContactModel:
        return CrisisContactModel(
            ...
            safety_plan_id=entity.safety_plan_id.value if entity.safety_plan_id else None,
            risk_level=entity.risk_level,
            warm_handoff=entity.warm_handoff,
            dispatched_at=ensure_utc(entity.dispatched_at) if entity.dispatched_at else None,
